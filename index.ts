...
app.get("/auth", async (req, res) => {
  res.redirect(
    "https://admin.obl.ong/oauth/authorize?" +
      new URLSearchParams({
...
app.get("/auth/callback", async (req, res) => {
  ...
  const json: any = await (
    await fetch("https://admin.obl.ong/oauth/token", {
      method: "POST",
      headers: {
  ...
  if (!json.access_token)
    return res.send("An error occured. Please nag david.").status(500);
  const token = json.access_token;

  const user: any = await (
    await fetch("https://admin.obl.ong/oauth/userinfo", {
      headers: {
...
app.get("/generate/pkpass", async (req, res) => {
  Sentry.startSpan({ name: "Generate Apple Wallet Pass" }, async (span) => {
    ...
    try {
      ...
      pass.barcodes({
        message: shajs("sha256").update(user.sub).digest("hex"),
